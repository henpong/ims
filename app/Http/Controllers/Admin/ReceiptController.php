...
class ReceiptController extends Controller
{
    ...
    public function transCreditReceipt(Request $request, $id){
        ...
        $invoicedetails = Receipt::with('branch')->where('customer_id',$id)->where('branch_id',$branchid)->where('status',3)->latest()->first();
        ...
        $invoice = Receipt::with(['branch','products','customers'])->where('customer_id',$id)->where('branch_id',$branchid)->whereDate('transaction_date',$date)->where('receipt_no',$receiptnum)->where('status',3)->latest()->get();
        ...
        // Calculate for total 
        $subtotal = Receipt::select('sub_total')->where('customer_id',$id)->whereDate('transaction_date',$date)->where('branch_id',$branchid)->where('receipt_no',$receiptnum)->where('status',3)->get();
        ...
        return view('layouts.admin.receipt.credit_receipt',['subtotal'=>$subtotal->sum('sub_total')])->with(compact('metaTitle','invoice','invoicedetails','payments'));
    }
